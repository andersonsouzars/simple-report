...
class TextField extends SRTextElements{
			...
			switch($elementName){
				...
				case 'textFieldExpression':
					...
					if($init == '$F{' && $fim == '}'){
						...
						$text = utf8_decode(SRParameter::get(substr($text, 3, -1)));
					}elseif($init == '$V{' && $fim == '}'){
						$text = (String)$element;
					}
	...
	public function draw(&$pdf, $o = null){
		
		if(is_array($o->report->variable))
			foreach ($o->report->variable as $v){
				if('$V{'.$v->name.'}' == $this->textFieldExpression)
					$this->textFieldExpression = $v->value;
			}
